...
class Version010013Date20260410000000 extends SimpleMigrationStep
{
    public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper
    {
        ...
        $jobs = $schema->hasTable('org_rb_jobs') ? $schema->getTable('org_rb_jobs') : $schema->createTable('org_rb_jobs');
        ...
        if (!$jobs->hasColumn('source_backup_job_id')) {
            $jobs->addColumn('source_backup_job_id', Types::BIGINT, ['notnull' => true]);
        }
        if (!$jobs->hasColumn('requested_by_uid')) {
            $jobs->addColumn('requested_by_uid', Types::STRING, ['notnull' => true, 'length' => 64]);
        }
        if (!$jobs->hasColumn('mode')) {
            $jobs->addColumn('mode', Types::STRING, ['notnull' => true, 'length' => 16, 'default' => 'dry_run']);
        }
        ...
        if (!$jobs->hasColumn('pre_restore_backup_job_id')) {
            $jobs->addColumn('pre_restore_backup_job_id', Types::BIGINT, ['notnull' => false, 'default' => null]);
        }
        ...
        if (!$jobs->hasIndex('obr_jobs_org_status') && !$jobs->hasIndex('org_backup_rollback_jobs_org_status_idx')) {
            $jobs->addIndex(['organization_id', 'status'], 'obr_jobs_org_status');
        }
        if (!$jobs->hasIndex('obr_jobs_status_id') && !$jobs->hasIndex('org_backup_rollback_jobs_status_id_idx')) {
            $jobs->addIndex(['status', 'id'], 'obr_jobs_status_id');
        }
        if (!$jobs->hasIndex('obr_jobs_org_source_mode') && !$jobs->hasIndex('org_backup_rollback_jobs_org_source_mode_idx')) {
            $jobs->addIndex(['organization_id', 'source_backup_job_id', 'mode'], 'obr_jobs_org_source_mode');
        }

        $steps = $schema->hasTable('org_rb_steps') ? $schema->getTable('org_rb_steps') : $schema->createTable('org_rb_steps');
        ...
        if (!$steps->hasIndex('obr_steps_job_step') && !$steps->hasIndex('org_backup_rollback_steps_job_step_uidx')) {
            $steps->addUniqueIndex(['job_id', 'step_key'], 'obr_steps_job_step');
        }
        if (!$steps->hasIndex('obr_steps_job_status') && !$steps->hasIndex('org_backup_rollback_steps_job_status_idx')) {
            $steps->addIndex(['job_id', 'status'], 'obr_steps_job_status');
        }

        $events = $schema->hasTable('org_rb_events') ? $schema->getTable('org_rb_events') : $schema->createTable('org_rb_events');
        ...
        if (!$events->hasIndex('obr_events_job_seq') && !$events->hasIndex('org_backup_rollback_events_job_seq_idx')) {
            $events->addIndex(['job_id', 'sequence_no'], 'obr_events_job_seq');
        }
        if (!$events->hasIndex('obr_events_job') && !$events->hasIndex('org_backup_rollback_events_job_idx')) {
            $events->addIndex(['job_id'], 'obr_events_job');
        }
